        ...
        await withTransaction(async (session) => {
          ...
          if (!isAdmin) {
            ...
            for (const phone of phoneNumbers) {
              ...
              // 기존 레코드 찾기
              const existingIndex = user.myRecords.findIndex(r => r.phoneNumber === phone);
              ...
              if (existingIndex >= 0) {
                // 업데이트
                user.myRecords[existingIndex] = newRecord;
                updatedCount++;
              } else {
                // 추가
                user.myRecords.push(newRecord);
                addedCount++;
              }
            }

            console.log(`레코드 업데이트: ${updatedCount}개, 추가: ${addedCount}개`);
            console.log(`총 myRecords 개수: ${user.myRecords.length}개`);
